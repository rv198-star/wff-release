...
def append_unique_flow(flows: list[dict[str, object]], title: str, steps: list[str]) -> None:
    ...
    if not cleaned_title or is_generic_flow_container_title(cleaned_title):
        return
    cleaned_steps = [
...
def extract_heading_defined_flows(source_text: str) -> list[dict[str, object]]:
    ...
    while idx < len(lines):
        ...
        if is_generic_flow_container_title(heading_title):
            ...
            continue
        flow_title = re.sub(
            r"^(?:主流程|核心业务流程|Key Business Flows?|Business Flows?|Main Flow)\s*[:：-]?\s*",
            "",
            heading_title,
            flags=re.IGNORECASE,
...
def extract_heading_block_any_level(source_text: str, heading_candidates: list[str], min_level: int = 2, max_level: int = 4) -> str:
    lines = source_text.splitlines()
    for index, line in enumerate(lines):
        ...
        if level < min_level or level > max_level:
            continue
        title = match.group(2)
        if not any(candidate.casefold() in title.casefold() for candidate in heading_candidates):
            continue
        block = [line]
        for child in lines[index + 1 :]:
            child_match = re.match(r"^(#{2,6})\s+", child.strip())
            if child_match and len(child_match.group(1)) <= level:
                break
            block.append(child)
        return "\n".join(block).strip()
    return ""
...
def extract_source_role_signals_any_level(source_text: str) -> list[str]:
    candidates: list[str] = []
    for pattern in (
        r"-\s+canonical_primary_actor:\s+`?([^`\n]+)`?",
        r"-\s+canonical_downstream_actor:\s+`?([^`\n]+)`?",
        r"-\s+Primary users:\s+([^.\n]+)",
        r"-\s+Downstream users:\s+([^.\n]+)",
    ):
        candidates.extend(re.findall(pattern, source_text, flags=re.IGNORECASE))
    roles_block = extract_heading_block_any_level(
        source_text,
        ["User, Buyer, Operator, And Decision Roles", "Target Users", "目标用户"],
    )
    for row in parse_markdown_table(roles_block):
        role = row.get("role") or row.get("user") or row.get("persona") or row.get("target_user") or ""
        if role:
            candidates.append(role)
    return list(dict.fromkeys(clean_runtime_label_item(candidate) for candidate in candidates if clean_runtime_label_item(candidate)))[:5]


def extract_source_module_signals_any_level(source_text: str) -> list[str]:
    modules_block = extract_heading_block_any_level(
        source_text,
        ["Module Responsibility Matrix", "Information Architecture Spec Matrix", "Module Matrix"],
    )
    modules = [
        table_row_value(row, "module", "module_name", "domain", "screen_module", "workflow_step")
        for row in parse_markdown_table(modules_block)
    ]
    return list(dict.fromkeys(clean_runtime_label_item(module) for module in modules if clean_runtime_label_item(module)))[:6]
...
def extract_key_business_flows_from_brief(
    ...
    flows: list[dict[str, object]] = []
    if block:
        first_heading = re.match(r"^(#+)\s+(.+?)\s*$", block.splitlines()[0].strip())
        container_level = len(first_heading.group(1)) if first_heading else 2
        container_title = first_heading.group(2).strip() if first_heading else ""
        current_title = ""
        current_steps: list[str] = []
        for line in block.splitlines()[1:]:
            heading = re.match(r"^(#{2,6})\s+(.+)$", line.strip())
            if heading and len(heading.group(1)) > container_level:
                if current_title:
        ...
        if current_title:
            append_unique_flow(flows, current_title, current_steps[:])
        if not flows and not is_generic_flow_container_title(container_title):
            append_unique_flow(flows, container_title, list_items_from_markdown(block))
...
def render_dynamic_flowchart(runtime_context: dict[str, object]) -> list[str]:
    ...
    node_ids = [f"FLOW_{idx}" for idx in range(1, len(names) + 1)]
    for node_id, name in zip(node_ids, names):
        flow_id = node_id.replace("_", "-")
        lines.append(f"    {node_id}[{flow_id}: {name}]")
...
def build_dynamic_signal_snapshot(source_text: str) -> dict[str, list[str]]:
    ...
    if not roles:
        roles = extract_source_role_signals_any_level(fact_text)
    if not modules:
        modules = extract_source_module_signals_any_level(fact_text)
...
def clean_operational_flow_step_text(raw_step: object) -> str:
    cleaned = re.sub(r"\s+", " ", str(raw_step)).strip()
    cleaned = re.sub(r"^(?:[-*]\s*)?(?:step\s*)?\d+[\).、:：]\s*", "", cleaned, flags=re.IGNORECASE).strip()
    return cleaned
...
def canonical_operational_flow_steps(runtime_context: dict[str, object]) -> list[str]:
    ...
    for row in runtime_context.get("ia_matrix", []):
        ...
        if not module or looks_like_placeholder(module):
            continue
        if responsibility and not looks_like_placeholder(responsibility):
            cleaned = f"{module}: {responsibility}"
        else:
            cleaned = module
        key = normalized_match_key(cleaned)
        if key in seen:
...
def bounded_flow_ref(index: int, flow_count: int) -> str:
    max_flow_ref = max(int(flow_count or 0), 1)
    return f"Flow {min(max(int(index or 1), 1), max_flow_ref)}"


def normalize_trace_flow_ref(value: str, *, flow_count: int | None = None) -> str:
    match = re.search(r"\b(?:FLOW|Flow|Step)[-_\s]*0*(?P<num>\d+)\b", value or "")
    if match:
        return bounded_flow_ref(int(match.group("num")), flow_count or int(match.group("num")))
    return value


def flow_mapping_reference_line(runtime_context: dict[str, object]) -> str:
    step_count = max(len(canonical_operational_flow_steps(runtime_context)), 1)
    span = "步骤 1" if step_count == 1 else f"步骤 1 到步骤 {step_count}"
    return (
        f"- flow_mapping: {span} 由 Operational Flow Specification、Acceptance Criteria "
        "和 Requirement Trace Matrix 共享同一 canonical flow set。"
    )


def payload_flow_mapping_reference_line(runtime_context: dict[str, object]) -> str:
    step_count = max(len(canonical_operational_flow_steps(runtime_context)), 1)
    return (
        f"- flow_mapping: payload、handoff 与 closure 引用必须落在 canonical Flow 1 到 Flow {step_count}；"
        "未渲染的静态 Step 编号不能作为交接依据。"
    )
...
def render_operational_flow_spec_lines(runtime_context: dict[str, object]) -> list[str]:
    ...
    for idx, line in enumerate(lines, start=1):
        ...
        if step_match:
            rendered_lines.append(f"{step_match.group('num')}. FLOW-{idx}: {step_match.group('body')}")
        else:
            rendered_lines.append(f"{idx}. FLOW-{idx}: {line}")
...
def flow_step_owner_surface(runtime_context: dict[str, object], step_text: object) -> str:
    ...
    for role in roles:
        ...
        role_alias = ""
        alias_match = re.search(r"[（(]([^）)]+)[）)]", role)
        if alias_match:
            role_alias = alias_match.group(1).strip()
        if role_label and role_label in step:
            return role
        if role_alias and role_alias.casefold() in lowered:
            return role

    system_owned = lowered.startswith("system ") or step.startswith("系统")
    if system_owned:
        return "System"
    ...
    if any(cue in lowered or cue in step for cue in decision_cues):
        return continuation_owner_surface(runtime_context)

    return str(runtime_context.get("primary_segment", "primary actor")).strip() or "primary actor"
...
def reader_facing_context_driver_line(label: str, runtime_context: dict[str, object]) -> str:
    ...
    primary_segment = plain_role_surface(
        str(runtime_context.get("primary_segment", "")).strip() or product_source_driver_primary_user(runtime_context)
    )
    ...
    if continuation_owner.casefold() == primary_segment.casefold():
        decision_clause = "并支持继续 / 调整 / 暂停判断"
    else:
        decision_clause = f"并让 {continuation_owner} 基于证据判断继续 / 调整 / 暂停"
    if label == "产品押注":
        return f"首版押注是把 {module_surface} 收成一条可复盘主线，让 {primary_segment} 能连续推进关键动作，{decision_clause}。"
    if label == "为什么现在":
...
def product_source_driver_primary_user(runtime_context: dict[str, object]) -> str:
    summary = product_source_driver_summary(runtime_context)
    product_judgment = summary.get("product_judgment", {})
    if isinstance(product_judgment, dict):
        return compact_signal_line(str(product_judgment.get("primary_user_or_buyer", "")))
    return ""


def product_source_driver_continuation_owner(runtime_context: dict[str, object]) -> str:
    summary = product_source_driver_summary(runtime_context)
    commercial_judgment = summary.get("commercial_judgment", {})
    if isinstance(commercial_judgment, dict):
        owner = compact_signal_line(str(commercial_judgment.get("continuation_owner", "")))
        if owner:
            return owner
    completeness_driver = summary.get("business_completeness_driver", {})
    economics = completeness_driver.get("continuation_economics", {}) if isinstance(completeness_driver, dict) else {}
    if isinstance(economics, dict):
        return compact_signal_line(str(economics.get("continuation_owner", "")))
    return ""
...
def continuation_owner_surface(runtime_context: dict[str, object]) -> str:
    ...
    driver_owner = product_source_driver_continuation_owner(runtime_context)
    if driver_owner:
        return driver_owner
...
def render_requirement_trace_matrix_from_rows(
    ...
    *,
    ...
    flow_by_ac_id = {
        str(row[0]).strip(): normalize_trace_flow_ref(str(row[10]).strip(), flow_count=flow_count)
        for row in acceptance_rows
        if len(row) > 10 and str(row[0]).strip()
    }
    rows: list[list[str]] = []
    for idx, (req_id, req_row) in enumerate(req_index.items(), start=1):
        ac_id = ac_ids[min(idx - 1, len(ac_ids) - 1)] if ac_ids else ""
        rows.append(
            [
                req_row[1],
                req_row[2],
                ...
                flow_by_ac_id.get(ac_id, bounded_flow_ref(idx, flow_count or 1)),
            ]
        )
    return markdown_table(
        [
...
def main() -> int:
    ...
    parser.add_argument("--claim-input-output", help="optional pre-render claim input JSON for PRD claim-control")
