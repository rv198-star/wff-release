"""P2 adapter binding bounded challenge evidence to the exact architecture decision."""
...
CONTRACT_ID = "p2-architecture-disposition-authority-contract.v1"
CONTRACT_DIGEST = canonical_digest(
    {
        ...
        "truth_boundary": "architecture judgment stays Agentic; generated names and Trace co-occurrence are not authority",
    }
)
...
def _context_rows(candidate: Mapping[str, Any]) -> list[dict[str, str]]:
    ...
    rows.append(
        {
            "identity": "p2-agentic-architecture-candidate",
            "sha256": str(candidate.get("content_digest") or ""),
        }
    )
    p1 = candidate.get("p1_authority")
    if isinstance(p1, Mapping):
        rows.append(
            {
                "identity": "accepted-p1-authority",
                "sha256": str(p1.get("authority_digest") or ""),
            }
        )
    return rows


def p2_decision_subject(
    *,
...
def p2_review_subject(
    *,
    ...
    return {
        "artifact": {
            "identity": f"p2-decision-subject:{decision_subject.get('decision_id')}",
...
def build_p2_decision_challenge_binding(
    *,
    candidate: Mapping[str, Any],
    decision_subject: Mapping[str, Any],
    ...
    return build_decision_challenge_binding(
        ...
        challenge=challenge,
        review_subject=p2_review_subject(candidate=candidate, decision_subject=decision_subject),
    )


def validate_p2_decision_challenge_binding(
    *,
    decision: Mapping[str, Any],
    candidate: Mapping[str, Any],
    required_trigger_ids: Iterable[str],
) -> dict[str, Any]:
    subject = p2_decision_subject(
        candidate=candidate,
    ...
    return validate_decision_challenge_binding(
        decision=decision,
        expected_review_subject=p2_review_subject(candidate=candidate, decision_subject=subject),
