...
Legacy first-version runner: full-pack/source-tree compatibility only. Slim install profiles ship the aggregate runner and focused capability runners instead.
...
Default slim implementation delivery profile:
...
The legacy foundation/project scaffold and direct converter sequence is full-pack/source-tree compatibility only. In slim install profiles, use the aggregate runner and focused capability runners above instead of shipping individual legacy scaffold/converter scripts.

Optional lanes:
- frontend lane:
  - `scripts/phase3/run_impl.py --enable-frontend`
- multi-worker / dispatch lane:
  - dispatch runner commands are full-pack/source-tree only unless a profile explicitly ships the dispatch sidecar.
- hardening / side review lane:
  - `scripts/phase3/run_impl_api_docs.py` for slim API-doc generation
  - review/security/coverage support modes are full-pack/source-tree only unless a profile explicitly ships the support sidecars
  - `scripts/phase3/phase3_toolchain_bootstrap.py --workspace-root <phase3-output-dir>`
...
For the first Phase-3 implementation package, prefer:
- `scripts/phase3/run_impl.py`
- for runtime-closure validation in slim profiles, run focused capability runners, bootstrap with `scripts/phase3/phase3_toolchain_bootstrap.py`, then use `scripts/phase3/phase3_delivery_gate.py` with supplied runtime evidence. The legacy first-version strict-runtime runner remains available only from full-pack/source-tree installs.
