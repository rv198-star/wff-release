...
`wff-validation` is the only public Phase-4 skill entrypoint. Do not ask users to install or invoke separate Phase-4 stage skills.

Keep the three default Stage-01 to Stage-03 responsibilities distinct inside this skill:

- acceptance planning: freeze the acceptance catalog, contract registry, Phase-2 decision alignment, gate posture, and visual/manual-evidence posture from the supplied Phase-3 root
- evidence execution: consume real inherited test/runtime/UI evidence, record per-item outcomes, and keep missing UI/visual/manual evidence explicit as `review-bound` or `blocked`
- closure judgment: return on functional failure, cap claims when evidence is missing, emit routed remediation packets, and state downstream reliance boundaries

This split is a control model, not a public skill family. It must not change Phase-4 stage order, output contracts, read-only validation behavior, or optional Stage-04 opt-in rules.
