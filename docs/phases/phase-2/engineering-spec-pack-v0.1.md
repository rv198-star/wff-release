...
### F.0 Existing-System Architecture Change Addendum

When Phase-2 consumes a `P2 Existing-System Architecture Change Intake Packet`, the ESP must include an `Existing-System Architecture Change Addendum`.

The addendum is not a new phase artifact. It is the visible result of the side branch's two Agent-owned actions:
...
- `change_impact_level`
- `change_mode`
- impacted existing contracts
- compatibility_strategy
- migration_strategy
- rollback_strategy
- decision_gate_status
- open architecture gaps and owner
- P3 handoff constraints

For `AC-1`, the ESP may state direct design readiness when validation points are explicit.

For `AC-2`, the ESP must include compatibility, migration, and rollback strategy before implementation-facing handoff.

For `AC-3`, the ESP must not mark the package ready for P3 unless an explicit architecture decision is recorded and destructive or contract-breaking risk is explicitly bounded. Owner confirmation is optional evidence; when no owner is available, the ESP must preserve that absence as review-bound and choose a conservative compatibility-preserving path.

For `AC-4`, the ESP must mark the normal implementation path blocked or routed to a separate architecture decision path.

P3 must not silently infer or downgrade unresolved existing-system gaps. Any missing architecture truth must remain review-bound or blocked with owner, validation path, and downstream usage rule.
...
- when an existing-system architecture change sidecar is present, the ESP includes an `Existing-System Architecture Change Addendum`
- `AC-3 / AC-4` sidecar cases are not marked implementation-ready without an explicit architecture decision gate
- compatibility, migration, rollback, and decision-gate state are visible before P3 handoff
