...
1. `PX-SK-01 codebase-baseline-extraction`
2. `PX-SK-04 technical-health-assessment`
3. `PX-SK-07 safety-net-test-construction`
4. `PX-SK-06 gap-analysis-and-change-decomposition` 的 `partial` 模式

不纳入 Wave-1：

- `PX-SK-02 database-baseline-extraction`
- `PX-SK-03 business-architecture-discovery`
- `PX-SK-05 target-architecture-design`
- `PX-SK-08 refactoring-strategy-definition`
- `PX-SK-09 migration-strategy-and-cutover-plan`
- `PX-SK-10 multi-system-entity-mapping`
...
- `PX-SK-01` 回答“现在是什么”
- `PX-SK-04` 回答“现在哪里最危险”
- `PX-SK-07` 回答“动刀前怎么先上护网”
- `PX-SK-06 partial` 回答“局部变更如何转成可被 P1/P3 消费的任务或约束”
...
`PX-SK-01 → PX-SK-04 → PX-SK-07 → P3 → P4`

### 场景 B：局部需求变更

路径：

`PX-SK-01 (局部) → PX-SK-06 partial → P1 → P2 → P3 → P4`
...
- P1 接受来自 `PX-SK-06 partial` 的变更需求包
- 变更需求包中保留：
...
落 `PX-SK-01` 与 `PX-SK-04` 的 stage package 4 件套。

### Step 3

补 `PX-SK-07`，让“评估完但无法安全开改”的场景有出口。

### Step 4

补 `PX-SK-06 partial`，打通“局部变更 → P1/P3”桥。
