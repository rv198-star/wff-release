...
这份文档先回答一个问题：WFF 到底由哪些入口、路线和产物层组成。
...
| 已有材料，想生成 PRD | `wff-req` | PRD、评分、验收结果 |
| 已有 PRD，想做系统设计 | `wff-arch` | `engineering-spec-pack.md`、`phase-3-implementation-entry.md` |
| 已有设计，想实现和测试 | `wff-impl` | 实现任务卡、代码、测试报告、交付检查 |
| 已有 P3 产物，想判断能不能收口 | `wff-validation` | closure summary、validation report、claim ceiling |
| 接手旧系统、迁移、重构、扩容 | `wff-x` | 代码/数据库/业务基线、风险、安全网、目标变化 |
| 想按工作角色使用 | `wff-agent` | 平台本地角色文件和角色职责边界 |

如果不确定，先问两个问题：

1. 这是新项目，还是已有系统改造？
2. 我现在缺的是需求、架构、实现、验证，还是角色化入口？
