...
from phase2.phase2_quality_check import analyze_stage, mermaid_blocks, mermaid_counts, profile_minimum, read_text
...
PROFILE_BACKED_EXPECTED_KEYS = {
    "stage_02": {"stateDiagram-v2": "stage_02_state_diagrams"},
    "stage_04": {"sequenceDiagram": "stage_04_sequence_diagrams"},
}


def expected_types_for_profile(stage_key: str, complexity_profile: str) -> dict[str, int]:
    expected = dict(EXPECTED_TYPES.get(stage_key, {}))
    for diagram_type, minimum_key in PROFILE_BACKED_EXPECTED_KEYS.get(stage_key, {}).items():
        expected[diagram_type] = profile_minimum(complexity_profile, minimum_key)
    return expected
...
def validate_stage(
    ...
    *,
    ...
    missing = []
    for diagram_type, minimum in expected_types_for_profile(stage_key, complexity_profile).items():
...
def main() -> int:
    ...
    parser.add_argument("--complexity-profile", default="standard", choices=("micro", "standard", "complex"))
