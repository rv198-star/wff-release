    ...
    *,
    ...
    if missing_from_index:
        lines.append(
            ...
            "is already wired into stage source-cards but is absent from `docs/source-registers/product-requirements-source-index.md`."
        )
    if missing_from_ledger:
        lines.append(
            f"- `source-unit ledger drift`: {', '.join(f'`{item}`' for item in missing_from_ledger)} "
            "is stage-wired but not represented in `docs/source-registers/phase-1-source-unit-coverage-ledger-v0.1.md`."
        )
...
def main() -> int:
    ...
    source_index_path = repo_root / "docs/source-registers/product-requirements-source-index.md"
    coverage_ledger_path = repo_root / "docs/source-registers/phase-1-source-unit-coverage-ledger-v0.1.md"
