...
def typescript_behavior_card_payload_helper_lines(*, map_name: str | None = None) -> list[str]:
    ...
    return [
        ...
        "    }",
        "    if (key === 'trace_id') {",
        "      enriched.trace_id = `trace-${operationId}`;",
        "    } else if (key === 'tenant_id') {",
        "      enriched.tenant_id = authContext.tenant_id ?? enriched.tenantId ?? 'tenant-001';",
        "    } else if (key === 'version') {",
        "      enriched.version = 1;",
        "      enriched.expectedVersion = enriched.expectedVersion ?? 1;",
        "    } else if (key.endsWith('_id')) {",
        "      enriched[camelKey] = `${key.replace(/_/g, '-')}-001`;",
        "    } else {",
        "      enriched[key] = `${key}-sample`;",
        "    }",
        "  }",
        "  return enriched;",
        "}",
...
def response_field_source_expr(field: str, evidence_keys: list[str]) -> str:
    normalized_field = snake_case(field)
    camel = camel_field(normalized_field)
    sources = [f"nextState.{field}", f"context.{field}"]
    for alias in (normalized_field, camel):
        if alias == field:
            continue
        sources.extend([f"nextState.{alias}", f"context.{alias}"])
