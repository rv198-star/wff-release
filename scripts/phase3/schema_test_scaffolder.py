...
def render_schema_test(table: dict[str, object]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"const uniqueConstraints: readonly string[] = {json.dumps(unique_constraints, ensure_ascii=False, indent=2)};",
            f"const compositeIndexes: readonly string[] = {json.dumps(composite_indexes, ensure_ascii=False, indent=2)};",
            "",
