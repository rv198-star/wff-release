...
def apply_thinking_value_gain_full_use(
    ...
    *,
    ...
    model["thinking_value_gain"] = {
        ...
        "boundary": profile_copy["boundary"],
        "exit_rule": profile_copy["exit_rule"],
    }
    ...
    arena["output_profile"] = output_profile
    ...
    draft["output_profile"] = output_profile
    draft["delivery_bias"] = profile_copy["delivery_bias"]
    draft["delivery_handoff_items"] = delivery_handoff_items
    ...
    draft["why_substitute_is_not_enough"] = (
        f"{profile_copy['substitute_prefix']}: {primary_substitute} is not enough because it can leave the buyer/operator with signals or records but without the proof chain "
        f"needed to decide continue / revise / pause and to hand P2 a designable operating boundary."
    )
    draft["proof_that_changes_decision"] = (
        f"{profile_copy['proof_prefix']}: {proof_target}; it must be explicit enough for {continuation_owner} to make a continue / revise / pause investment decision."
    )
    ...
    draft["architecture_pressure"] = (
        f"{profile_copy['architecture_prefix']}: P2 must preserve the source-grounded value path from {boundary} through evidence, review, and handoff; "
    ...
    thesis["output_profile"] = output_profile
    thesis["delivery_bias"] = profile_copy["delivery_bias"]
    thesis["delivery_handoff_items"] = delivery_handoff_items
    ...
    release_truth["thinking_value_gain_output_profile"] = output_profile
