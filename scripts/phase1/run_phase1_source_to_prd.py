#!/usr/bin/env python3
"""Phase-1 source-to-PRD orchestration runner."""
...
P1_SOURCE_PACKET_PATTERN = re.compile(r"^#\s+P1 Source Input Packet\b", flags=re.IGNORECASE | re.MULTILINE)
P1_SOURCE_BRIEF_PACKET_HEADING_PATTERN = re.compile(r"^##\s+P1 Source Brief\b", flags=re.IGNORECASE | re.MULTILINE)
SOURCE_ADMISSION_SIGNAL_PATTERNS: dict[str, re.Pattern[str]] = {
    "role": re.compile(
        r"\b(?:target user|user|buyer|operator|role|coordinator|lead|reviewer|manager|owner)\b|用户|角色|负责人|评审|运营",
        flags=re.IGNORECASE,
    ),
    "problem": re.compile(
        r"\b(?:problem|pain|current|status quo|lose|missing|blocked|gap|risk)\b|问题|痛点|现状|缺失|阻塞|风险|无法|难以|分散|缺口|低效|耗时",
        flags=re.IGNORECASE,
    ),
    "goal": re.compile(
        r"\b(?:goal|outcome|success|desired|reduce|make|improve|increase)\b|目标|结果|成功|降低|提升|达成",
        flags=re.IGNORECASE,
    ),
    "workflow": re.compile(
        r"\b(?:workflows?|flows?|scenarios?|process|step|mvp|scope|loop|intake|inspection|closure|handoff)\b|流程|步骤|范围|闭环|交接|链路|输入|生成|提交|修改|创建|评审",
        flags=re.IGNORECASE,
    ),
    "evidence": re.compile(
        r"\b(?:validation|signal|evidence|proof|metric|review|acceptance)\b|验证|信号|证据|指标|验收|复盘|采纳|使用|下降|减少",
        flags=re.IGNORECASE,
    ),
    "constraint": re.compile(
        r"\b(?:constraint|unknown|out of scope|boundary|non-goal|assumption)\b|约束|未知|边界|非目标|假设",
        flags=re.IGNORECASE,
    ),
}


def _is_url_only_source(source_text: str) -> bool:
    compact = re.sub(r"\s+", " ", source_text).strip()
    if not compact or not URL_PATTERN.search(compact):
        return False
    without_urls = URL_PATTERN.sub("", compact)
    without_markdown_noise = re.sub(r"[\s\[\]()<>{}`*_#:\-./]+", "", without_urls)
    return not without_markdown_noise


def _phase1_admission_fact_surface(source_text: str) -> tuple[str, bool, bool]:
    packet_detected = bool(P1_SOURCE_PACKET_PATTERN.search(source_text))
    if not packet_detected:
        return source_text, False, False
    packet_has_brief = bool(P1_SOURCE_BRIEF_PACKET_HEADING_PATTERN.search(source_text))
    block = source_fact_text(source_text) if packet_has_brief else ""
    return block, True, packet_has_brief


def _meaningful_source_lines(fact_text: str) -> list[str]:
    meaningful: list[str] = []
    for raw_line in fact_text.splitlines():
        line = raw_line.strip()
        if not line:
            continue
        if line.startswith("#"):
...
def _phase1_source_signal_groups(fact_text: str) -> list[str]:
    return [
        name
        for name, pattern in SOURCE_ADMISSION_SIGNAL_PATTERNS.items()
        if pattern.search(fact_text)
    ]


def _phase1_source_structure_groups(fact_text: str, signal_groups: list[str]) -> list[str]:
    context = extract_domain_context(fact_text)
    groups: list[str] = []
    if context.get("roles") or context.get("segments"):
        groups.append("role")
    if context.get("objects"):
        groups.append("object")
    if context.get("flows") or "workflow" in signal_groups:
        groups.append("route")
    return groups


def evaluate_phase1_source_admission(source_text: str) -> dict[str, Any]:
    """Evaluate whether a Phase-1 source carries enough facts to generate from.

    This gate protects the runner boundary. It intentionally rejects URL-only
    references and source-input packets whose `P1 Source Brief` is missing or
    empty, because later stages can otherwise fill a polished generic PRD.
    """
    ...
    if not stripped:
        return {
            ...
            "packet_has_p1_source_brief": False,
            "admission_state": "",
        }
    if _is_url_only_source(raw_text):
        return {
            "allowed": False,
            "reason": "url_only_source",
            "fact_char_count": 0,
            "meaningful_line_count": 0,
            "signal_group_count": 0,
            "signal_groups": [],
    ...
    fact_text, packet_detected, packet_has_brief = _phase1_admission_fact_surface(raw_text)
    admission_state = source_packet_admission_state(raw_text) if packet_detected else ""
    if packet_detected and not packet_has_brief:
        return {
            "allowed": False,
            "reason": "source_packet_missing_p1_source_brief",
            "fact_char_count": 0,
            "meaningful_line_count": 0,
            "signal_group_count": 0,
            "signal_groups": [],
    ...
    meaningful_lines = _meaningful_source_lines(fact_text)
    fact_body = "\n".join(meaningful_lines).strip()
    fact_char_count = len(fact_body)
    signal_groups = _phase1_source_signal_groups(f"{fact_text}\n{fact_body}")
    structure_groups = _phase1_source_structure_groups(fact_text, signal_groups)
    result = {
        "allowed": True,
        "reason": "ok",
        "fact_char_count": fact_char_count,
        ...
        "packet_has_p1_source_brief": packet_has_brief,
        "admission_state": admission_state,
    }
    if admission_state == "not-admission-ready":
        result["allowed"] = False
        result["reason"] = "source_packet_not_admission_ready"
        return result
    if len(meaningful_lines) < 3:
        result["allowed"] = False
        result["reason"] = "thin_source_meaningful_lines"
        return result
    if fact_char_count < 120:
        result["allowed"] = False
        result["reason"] = "thin_source_fact_surface"
        return result
    if len(signal_groups) < 3:
        result["allowed"] = False
        result["reason"] = "missing_business_signal_groups"
        return result
    if "role" not in structure_groups:
        result["allowed"] = False
        result["reason"] = "missing_source_role_facts"
        return result
    if "object" not in structure_groups:
        result["allowed"] = False
        result["reason"] = "missing_source_object_facts"
        return result
    if "route" not in structure_groups:
        result["allowed"] = False
        result["reason"] = "missing_source_route_facts"
        return result
    return result


def validate_phase1_source_admission(source: Path) -> dict[str, Any]:
    if not source.exists() or not source.is_file():
        raise ValueError(f"source file does not exist: {source}")
    result = evaluate_phase1_source_admission(source.read_text(encoding="utf-8", errors="ignore"))
    if result["allowed"]:
        return result
    raise ValueError(
        "source admission gate blocked: "
        f"{result['reason']} "
        f"(meaningful_lines={result['meaningful_line_count']}, "
        f"fact_chars={result['fact_char_count']}, "
        f"signal_groups={result['signal_group_count']})"
    )
...
class Phase1SourceToPrdContext:
    script_dir: Path
    repo_root: Path
    python: str
    source: Path
...
@dataclass(frozen=True)
class Phase1SourceToPrdResult:
    draft_gate_code: int
    final_gate_code: int


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the Phase-1 source-to-PRD orchestration")
    ...
    parser.add_argument("--owner", default="Codex Phase-1 source-to-PRD runner")
    ...
    parser.add_argument(
        "--skip-stage-02b",
        action="store_true",
        help="run the official Phase-1 source-to-PRD path with a rich Stage-02b skip-stub instead of complete Stage-02b deepening",
    )
    parser.add_argument(
        "--max-rounds",
        type=int,
...
def parse_phase1_source_to_prd_args(argv: list[str] | None = None) -> argparse.Namespace:
    return build_parser().parse_args(argv)


def build_phase1_source_to_prd_context(args: argparse.Namespace) -> Phase1SourceToPrdContext:
    ...
    return Phase1SourceToPrdContext(
...
def print_phase1_source_to_prd_start(context: Phase1SourceToPrdContext) -> None:
    print("== Phase-1 Source-to-PRD Run ==")
...
def build_phase1_stage_artifact_depth_gate_command(context: Phase1SourceToPrdContext) -> list[str]:
    return [
        context.python,
...
def build_phase1_assemble_prd_command(context: Phase1SourceToPrdContext) -> list[str]:
    return [
        context.python,
...
def build_phase1_converge_prd_command(context: Phase1SourceToPrdContext) -> list[str]:
    command = [
        context.python,
...
def build_phase1_localize_prd_command(context: Phase1SourceToPrdContext) -> list[str]:
    return [
        context.python,
...
def build_phase1_emit_depth_runtime_artifacts_command(context: Phase1SourceToPrdContext) -> list[str]:
    return [
        context.python,
...
def build_phase1_execution_report_command(context: Phase1SourceToPrdContext, *, gate_json_path: Path) -> list[str]:
    command = [
        context.python,
...
def build_phase1_skill_family_audit_command(context: Phase1SourceToPrdContext) -> list[str]:
    command = [
        context.python,
...
def build_phase1_material_library_audit_command(context: Phase1SourceToPrdContext) -> list[str]:
    return [
        context.python,
...
def build_phase1_prd_excellence_regression_command(context: Phase1SourceToPrdContext) -> list[str]:
    return [
        context.python,
...
def phase1_main_stage_commands(context: Phase1SourceToPrdContext) -> list[list[str]]:
...
def write_phase1_draft_report(context: Phase1SourceToPrdContext) -> None:
    write_draft_report(
...
def run_phase1_skill_family_audit(context: Phase1SourceToPrdContext, *, segments: list[dict[str, Any]] | None = None) -> None:
...
def run_phase1_material_library_audit(context: Phase1SourceToPrdContext, *, segments: list[dict[str, Any]] | None = None) -> None:
...
def run_phase1_postrun_audits(context: Phase1SourceToPrdContext, *, segments: list[dict[str, Any]] | None = None) -> None:
...
def _phase1_claim_source_candidates(context: Phase1SourceToPrdContext) -> list[Path]:
    return [
        context.stage_02b,
        context.stage_03,
        context.stage_04,
...
def _phase1_claim_input_payload(context: Phase1SourceToPrdContext) -> dict[str, Any]:
...
def _phase1_prd_claim_surface_payload(context: Phase1SourceToPrdContext, *, prd_text: str) -> dict[str, Any]:
...
def build_phase1_prd_claim_surface(context: Phase1SourceToPrdContext, *, prd_text: str) -> dict[str, Any]:
...
def emit_phase1_claim_control_sidecar(context: Phase1SourceToPrdContext) -> dict[str, Any]:
...
def run_phase1_source_to_prd(context: Phase1SourceToPrdContext) -> Phase1SourceToPrdResult:
...
def emit_phase1_source_to_prd_summary(context: Phase1SourceToPrdContext, result: Phase1SourceToPrdResult) -> int:
    ...
    review_index = context.output_dir / "human-review" / "INDEX.md"
    print(f"review_index: {review_index}")
    print(f"legacy_human_review_index: {review_index}")
...
Phase1FullTrialContext = Phase1SourceToPrdContext
Phase1FullTrialResult = Phase1SourceToPrdResult
parse_phase1_full_trial_args = parse_phase1_source_to_prd_args
build_phase1_full_trial_context = build_phase1_source_to_prd_context
print_phase1_full_trial_start = print_phase1_source_to_prd_start
run_phase1_full_trial = run_phase1_source_to_prd
emit_phase1_full_trial_summary = emit_phase1_source_to_prd_summary
