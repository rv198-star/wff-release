...
def join_blocks(*values: str) -> str:
    return "\n".join(value for value in values if value)


def aggregate_realization_corpus(analyses: dict[str, dict[str, Any]]) -> str:
    stage_03_blocks = analyses["stage_03"]["blocks"]
    stage_04_blocks = analyses["stage_04"]["blocks"]
    return join_blocks(
        stage_03_blocks["data_ownership_map"],
        stage_03_blocks["schema_draft"],
        stage_03_blocks["storage_strategy"],
        stage_03_blocks["access_pattern_and_index_strategy"],
        stage_03_blocks["interface_contracts"],
        stage_03_blocks["api_endpoint_draft"],
        stage_03_blocks["public_boundary_registry_closure"],
        stage_03_blocks["stage_02_event_name_carry_forward"],
        stage_03_blocks["interaction_flow"],
        stage_03_blocks["scenario_coverage_matrix"],
        stage_03_blocks["contract_trace_registry"],
        stage_04_blocks["design_verification_notes"],
        stage_04_blocks["verification_replay_evidence"],
        stage_04_blocks["unresolved_risks_and_review_bound_items"],
        stage_04_blocks["rbi_trace_registry"],
        stage_04_blocks["implementation_handoff_package"],
        stage_04_blocks["implementation_task_sketch"],
    )


def canonical_rows_for_object(object_name: str, canonical_object_rows: list[dict[str, str]]) -> list[dict[str, str]]:
    return [
        row
        for row in canonical_object_rows
        if names_overlap(object_name, row.get("object_name", ""))
        or names_overlap(object_name, row.get("authoritative_aggregate", ""))
    ]


def realization_refs_resolve(row: dict[str, str], corpus: str) -> bool:
    ...
    schema_ok = bool(schema_refs) and all(fuzzy_contains(corpus, ref, min_token_matches=1) for ref in schema_refs)
    boundary_ok = bool(boundary_refs) and all(fuzzy_contains(corpus, ref, min_token_matches=1) for ref in boundary_refs)
    return schema_ok or boundary_ok


def unrealized_aggregate_objects(
    object_names: list[str],
    canonical_object_rows: list[dict[str, str]],
    realization_corpus: str,
) -> list[str]:
    missing: list[str] = []
    for object_name in object_names:
        direct_realization = fuzzy_contains(realization_corpus, object_name, min_token_matches=1)
        canonical_realization = any(
            realization_refs_resolve(row, realization_corpus)
            for row in canonical_rows_for_object(object_name, canonical_object_rows)
        )
        if not (direct_realization or canonical_realization):
...
def analyze_consistency(stage_paths: dict[str, Path], phase1_prd: Path | None = None) -> dict[str, Any]:
    ...
    realization_corpus = aggregate_realization_corpus(analyses)
    unrealized_objects = unrealized_aggregate_objects(object_names, canonical_object_rows, realization_corpus)
    rows.append(
        build_row(
            "traceability",
            "Stage-02 aggregate/object realization coverage",
            not object_parse_issue and not unrealized_objects,
            (
                "parse_failure: no-parseable-object-names"
                if object_parse_issue
                else ", ".join(unrealized_objects)
            ),
            "give every Stage-02 aggregate/object a concrete Stage-03/04 realization: table, projection/read model, contract/API, event/log stream, external mapping, explicit non-persistent decision, or RBI deferral",
        )
    )
    rows.append(
        build_row(
            "diagnostic",
            "Schema table count vs Stage-02 aggregate/object count",
            True,
            f"schema_table_count={schema_table_count}, aggregate_count={aggregate_count}",
            "diagnostic only: table-count parity is not required when aggregate/object realization is explicit through another accepted path",
        )
    )
