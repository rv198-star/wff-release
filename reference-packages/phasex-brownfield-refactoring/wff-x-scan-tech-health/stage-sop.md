# wff-x-scan-tech-health SOP — scan-tech-health

## 1. Positioning

- goal: judge brownfield technical risk and readiness for change
- upstream: `wff-x-scan-code-baseline`
- downstream: human decision, `wff-x-plan-test-protection`, `wff-x-intake-target-driver`, or direct Phase-3 entry when risk is low
...
7. map each material risk to one action: `stop`, `assess-more`, `build-safety-net`, `proceed-technical-refactor`, or `package-target-driver`
...
- do not invent missing codebase truth that `wff-x-scan-code-baseline` did not establish
