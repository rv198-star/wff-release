...
## v1.3.1 Value-Bearing Runnable Closure
...
- At least one generated code, test, or evidence unit must change before v1.3.2 can claim generated-artifact quality gain.
- `script pass alone does not prove quality gain`; compare against v1.2.4 and v1.3.1 baselines and keep unresolved items review-bound.
- Do not use this loop to reopen P1 business truth, redesign P2 architecture, start v1.3.3 script slimming, or edit generated `/tmp` proof outputs by hand.
...
## v1.3.2 Semantic Invariant Repair
...
- Protected v1.3.2 quality-floor surfaces stay mandatory: runtime smoke, started-service smoke, phase verdict, delivery gate, verification ledger, WP gate, full-targeted evidence, Agentic generation quality loop, repair interrupt, semantic invariant support, and evidence-capped closure language.
- P1/P2 are read-only upstream pressure-audit inputs inside v1.3.3; do not modify P1 source-truth generation or P2 architecture generation from this slice.
- Diagnostic-only material may move under `.phase3-diagnostics/` when tests prove the canonical evidence path remains intact.
- `generated-runtime-positioning.md` is diagnostic in v1.3.3 and should be emitted as `.phase3-diagnostics/generated-runtime-positioning.md`, not as a root-level formal-state surface.
...
### v1.3.3.R1 P3 Aggressive Surface Profiles
...
### v1.3.3.R2/R3 Strict-Runtime And Final Review
...
### v1.3.3.R4 Slimming Success Patterns

Use v1.3.3 as a P3 pattern library, not as a mechanical cross-phase patch.
...
Focused tests prove `module_behavior_plan` derivation, service/repository behavior-depth audit, evidence-path emission, and failure-before-file-write only. `WO-119F.5R` later accepted only a bounded selected-module behavior-depth gain over F.4 after R6 release-packaged GEO/PetClinic P3 strict-runtime passed and Review compared the selected modules against v1.3.3 / v1.2 baselines.

Do not claim broad generated-artifact quality gain, v1.3.3 / v1.2 baseline parity, route promotion, P1/P2/P4 applicability, material project slimming, or broad `WO-119F / v1.3.6 Agent-led Mainline Full Reconstruction` from F.5 evidence. The route remains explicit opt-in, selected-module-only, and non-default.
...
F.8 release-packaged R2 evidence accepted this boundary after GEO and PetClinic P3 strict-runtime passed from the default route with selected-module mode disabled. The accepted F.8 claim is bounded: default P3 body authoring moved forward, read/list pseudo-write burden was reduced, and the strict-runtime floor held. Do not claim broad material generated-artifact quality gain, v1.3.3 / v1.2.4 baseline superiority, P1/P2/P4 applicability, production authorization, or release approval from F.8 evidence.
...
Use these release-facing references when applying the v1.3.1 Phase-3 control model:
