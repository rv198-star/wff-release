...
真正的控制边界仍然是：路线决定顺序，skills 执行能力，证据限制声明，AI / 外部 review 处理需要独立判断的结论。
...
- 如果运行产生 `human-review/` 或类似 review 阅读入口，优先从那里读。
...
| WFF 是自动化全链路脚本 | 自动化是执行面；skills、install profiles、Role Agents 和 AI / 外部 review 同样是使用面 |
