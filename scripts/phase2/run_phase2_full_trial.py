        ...
        rows.append(
            {
                ...
                "source_ids": [str(op_by_name.get(operation_id, {}).get("contract_trace_id", ""))],
                "related_operations": [operation_id],
                "related_schema_or_domain_objects": [],
                "target_path_hint": f"service/{operation_id}",
                "catalog_status": "defined" if op_by_name.get(operation_id) else "review-bound",
            }
        )
        component_index += 1
        ...
        rows.append(
            {
                ...
                "related_operations": [str(row.get("operation_id", "")) for row in operation_rows],
...
def build_component_action_card_obligation_rows(
    ...
    for component in catalog_rows:
        ...
        available = list(dict.fromkeys([item for item in component.get("source_ids", []) if str(item).strip()] + operation_source_ids))
...
def write_engineering_spec_pack(
    *,
    ...
    quick_start_items = [
        "- 1. Read `3.1 System Boundary Statement` and `3.6 Key Architecture Decisions` to recover the immutable boundary and decision posture.",
    ...
    if stage_02_5 and stage_02_5.exists():
        ...
        quick_start_items.append(
            f"- 7. Start implementation planning from {', '.join(f'`{row['wp_id']}`' for row in wp_registry[:3]) or '`WP registry missing`'} and preserve RBI ownership for {', '.join(f'`{row['rbi_id']}`' for row in rbi_registry[:3]) or '`RBI registry missing`'}."
        )
    else:
        quick_start_items.extend(
            [
                ...
                f"- 6. Start implementation planning from {', '.join(f'`{row['wp_id']}`' for row in wp_registry[:3]) or '`WP registry missing`'} and preserve RBI ownership for {', '.join(f'`{row['rbi_id']}`' for row in rbi_registry[:3]) or '`RBI registry missing`'}.",
            ]
        )
    ...
    sections = [
        ...
        f"- delivery_profile: `{profile}`",
        f"- upstream_phase1_prd: `{phase1_prd}`",
        "",
        "## 2. Upstream Source Set",
        f"- Stage-01 architecture entry: `{stage_01.name}`",
        f"- Stage-02 decomposition: `{stage_02.name}`",
        (
            f"- Stage-02.5 third-party integration lane: `{stage_02_5.name}`"
            if stage_02_5 and stage_02_5.exists()
            else "- Stage-02.5 third-party integration lane: `not-present`"
        ),
        f"- Stage-03 data/interface design: `{stage_03.name}`",
        f"- Stage-04 convergence package: `{stage_04.name}`",
        f"- execution report target: `{execution_report.name}`",
        "",
        "### 2.1 Quick-Start Reading Path",
        quick_start_path,
        "",
        ...
        "This Engineering Spec Pack is intentionally self-contained. It carries forward the system boundary, decision baseline, decomposition, schema/API, risks, and realizability judgment so an implementation team does not need to bounce back and forth across four separate Stage artifacts just to understand the first executable design posture.",
        "",
        "### 3.1 System Boundary Statement",
        render_block(stage_01_text, "system_boundary_statement"),
        "",
        "### 3.2 Constraint Posture",
        render_block(stage_01_text, "constraints"),
        "",
        "### 3.3 Thesis-Driven Architecture Handoff",
        "This section is intentionally near the top of the ESP: implementation should see the business proof loop before it sees module and API detail, so Phase-2 does not collapse back into CRUD, read-only dashboards, or record-only stores.",
        "",
        "#### 3.3A Chosen Thesis Architecture Pressure",
        render_block(stage_01_text, "chosen_thesis_architecture_pressure"),
        "",
        "#### 3.3B Thesis-Driven Translation",
        render_block(stage_01_text, "thesis_driven_architecture_translation"),
        "",
        "#### 3.3C Stage-04 Implementation Handoff",
        render_block(stage_04_text, "thesis_driven_architecture_handoff"),
        ...
        "",
        "### 3.4 Quality Attribute / NFR Absorption",
        render_block(stage_01_text, "quality_attribute_structure"),
        "",
        "### 3.5 Security Architecture Sketch",
        render_block(stage_01_text, "security_architecture_sketch"),
        "",
        "### 3.6 Capacity Estimation",
        render_block(stage_01_text, "capacity_estimation"),
        "",
        "### 3.7 Key Architecture Decisions",
        render_block(stage_01_text, "key_architecture_decisions"),
        "",
        "## 4. Decomposition and Ownership",
        "The implementation-facing ownership model remains the Phase-2 decomposition spine. Domains, modules, logical services, and collaboration boundaries are carried inline here so downstream work does not re-invent ownership from filenames or UI surfaces.",
        "",
        "### 4.1 Domain Map",
        render_block(stage_02_text, "domain_map"),
        "",
        "### 4.2 Module Map",
        render_block(stage_02_text, "module_map"),
        "",
        "### 4.3 Service Candidates",
        render_block(stage_02_text, "service_candidates"),
        "",
        "### 4.4 Responsibility Matrix",
        render_block(stage_02_text, "responsibility_matrix"),
        "",
        "### 4.5 Dependency / Collaboration Map",
        render_block(stage_02_text, "dependency_collaboration_map"),
        "",
        "### 4.6 Conceptual Entity Relationship Diagram",
        render_block(stage_02_text, "entity_relationship_diagram"),
        "",
        "### 4.7 Domain Event Catalog",
        render_block(stage_02_text, "domain_event_catalog"),
        "",
        *optional_stage_sections,
        "## 5. Schema Summary",
        "The schema summary below preserves table-level design inline. Full field-level detail still belongs to Stage-03, but this pack keeps the structural baseline visible for implementation planning and migration sequencing.",
        "",
        "### 5.1 Data Sensitivity and Compliance Matrix",
        "This matrix is implementation-facing: it makes PII posture, masking/encryption rules, retention, and audit access expectations explicit before schema, API, and observability work fork in different directions.",
        "",
        render_block(stage_03_text, "data_sensitivity_and_compliance_matrix"),
        "",
        "### 5.2 Schema Draft",
        render_block(stage_03_text, "schema_draft"),
        "",
        "## 6. API Summary",
        "Public contracts, endpoint shapes, and interaction flow are carried into the Engineering Spec Pack so controller and adapter work can start from explicit boundary truth rather than pointer-only references.",
        "",
        ...
        "",
        "### 6.7 Technology Stack and Deployment Assumptions",
        render_block(stage_03_text, "technology_stack_and_deployment_assumptions"),
        "",
        "### 6.8 Technology Selection Evaluation Matrix",
        render_block(stage_03_text, "technology_selection_evaluation_matrix"),
        "",
        "### 6.9 Dominant Bottleneck Hypothesis",
        render_block(stage_03_text, "dominant_bottleneck_hypothesis"),
        "",
        "### 6.10 Architecture Alternative Candidate Set",
        render_block(stage_03_text, "architecture_alternative_candidate_set"),
        "",
        "### 6.11 Baseline Insufficiency Note",
        render_block(stage_03_text, "baseline_insufficiency_note"),
        "",
        "### 6.12 Constraint-Dominant Optimum Candidate",
        render_block(stage_03_text, "constraint_dominant_optimum_candidate"),
        "",
        "### 6.13 Key Tradeoff Decisions",
        render_block(stage_03_text, "key_tradeoff_decisions"),
        "",
        "### 6.14 Capacity and Performance Assumptions",
        render_block(stage_03_text, "capacity_and_performance_assumptions"),
        "",
        "## 7. Key Decisions Quick Reference",
        render_block(stage_01_text, "key_architecture_decisions"),
        "",
        "## 8. Risk Summary",
        render_block(stage_04_text, "unresolved_risks_and_review_bound_items"),
        "",
        "## 9. Realizability Judgment",
        realizability_section,
        "",
        "## 10. Delivery Expression and Implementation Intake",
        frontend_surface_architecture,
        "",
        "### 10.2 Prototype or Structured Delivery Expression",
        render_block(stage_04_text, "prototype_or_structured_delivery_expression"),
        "",
        "### 10.3 Implementation Handoff Package",
        render_block(stage_04_text, "implementation_handoff_package"),
        "",
        "#### 10.3A Implementation Task Sketch",
        render_block(stage_04_text, "implementation_task_sketch"),
        "",
        "### 10.4 Implementation Start Order",
        "This section is implementation-specific rather than a Stage copy. It converts the Stage-04 work-package ledger into the concrete start order that Phase-3 should preserve during branch planning, staffing, and dependency management.",
        "",
        implementation_start_order,
        "",
        "### 10.5 Schema and Data Migration Focus",
        "The goal here is not to restate every Stage-03 schema row verbatim, but to call out the create/backfill sequence that implementation must preserve so contract and review flows do not start from partially initialized storage.",
        "",
        schema_migration_focus,
        "",
        "### 10.6 Contract Freeze and Adapter Boundaries",
        "Implementation may optimize internals, but it must not rename public contracts or silently weaken endpoint failure semantics. The surfaces below are the boundary freeze set for the first build.",
        "",
        contract_freeze,
        "",
        "### 10.6A Operation Source Obligation Matrix",
        "P2 is the source of truth for per-operation risk tier and required behavior-source types. P3 must consume this matrix and may recompute risk only to report mismatches, not to downgrade source requirements.",
        "",
        operation_source_obligation_markdown(operation_source_obligation_rows),
        "",
        "### 11.2.1 Operation Design Source Registry",
        operation_design_source_registry_markdown(operation_design_source_rows),
        "",
        "### 10.6B Implementation Depth Obligation Matrix",
        "P2 combines P1 business value, P2 engineering risk, and implementation complexity into ACD. P3 must consume these rows and may not downgrade missing P1/P2 source material to low depth.",
        "",
        generic_matrix_markdown(
        ...
        "",
        "### 10.6C Implementation Component Catalog",
        "P2 owns component identities before P3 writes code. Missing catalog rows are review-bound; P3 must not invent components from filenames alone.",
        "",
        generic_matrix_markdown(
        ...
        "",
        "### 10.6D Component Action Card Obligation Matrix",
        "Each Service/Domain/Repository/Adapter card obligation is derived from P1/P2 source rows. Source sufficiency is separate from required depth.",
        "",
        generic_matrix_markdown(
        ...
        "",
        "### 10.7 Operational Rollout Guardrails",
        "Open review-bound items remain implementation guardrails, not documentation leftovers. Branch planning, rollout sequencing, and release readiness must treat these RBI bindings as active constraints.",
        "",
        rollout_guardrails,
        "",
        "### 10.8 Observability and Operational Readiness",
        "Logs, metrics, alerts, SLO thresholds, and rollout guardrails must be designed before implementation fans out. This section is the minimum operational contract that implementation should preserve while building controllers, jobs, and adapters.",
        "",
        render_block(stage_04_text, "observability_and_operational_readiness"),
        "",
        "### 10.9 Identity, Auth Vendor, and Key Lifecycle",
        "When delivery realism depends on auth provider choice, token lifetime, refresh/rotation posture, or key storage, this section becomes part of the implementation baseline rather than an optional note.",
        "",
        render_block(stage_04_text, "identity_and_key_management_choice_posture"),
        "",
        "### 10.10 Phase-3 Onboarding Summary",
        "This section keeps the first-day implementation entry concise: what to read first, what terms matter, and what environment or dependency prerequisites must be preserved before coding begins.",
        "",
        onboarding_summary_block,
        "",
        "## 11. Traceability Runtime",
        f"- validation_reference: `{trace_validation.name}`",
        f"- registry_report_reference: `{trace_report_text.name}`",
        "- traceability_rule: preserve artifact ids, dependency chain, and unresolved truth from the canonical Phase-2 Stage outputs",
    ]
...
1. Read `{engineering_spec_pack.name}` sections `10.5` to `10.11` before opening implementation slices.
2. Start with `{", ".join(f"`{item}`" for item in start_wp_ids) or "`WP registry missing`"}` and preserve declared dependency order from Stage-04.
3. Carry unresolved review-bound ownership for `{", ".join(f"`{item}`" for item in start_rbi_ids) or "`RBI registry missing`"}` into implementation planning instead of silently dropping the RBI chain.
4. Bind new implementation artifacts to `IMPL-STG00-INPUT-0001` only after `{trace_validation.name}` remains `PASS`.
{extra_start_sequence}
...
- case_complexity_profile:
  - `{complexity_profile}`
- complexity_classifier_reference:
  - `{complexity_classification_path.name}`
- complexity_profile_suggested:
  - `{classification_suggested_profile}`
- complexity_profile_selection_mode:
  - `{classification_selection_mode}`
- complexity_profile_selection_confidence:
  - `{classification_confidence}`
- complexity_profile_override_justification:
  - `{classification_override_reason}`
- deployment_posture_suggested:
  - `{deployment_posture["suggested"]}`
- deployment_posture_selected:
  - `{deployment_posture["selected"]}`
- deployment_posture_selection_mode:
  - `{deployment_posture["selection_mode"]}`
- deployment_posture_warning_class:
  - `{deployment_posture["warning_class"]}`
- deployment_posture_override_source:
  - `{deployment_posture["override_source"]}`
- deployment_posture_override_reason:
  - `{deployment_posture["override_reason"]}`
- deployment_posture_added_risks:
  - `{deployment_posture["added_risks"]}`
...
## 1.1 Complexity Classification Basis
{classification_indicator_lines}

## 1.2 Deployment / Infrastructure Posture Basis
...
- deployment_posture_summary:
  - `{deployment_warning_summary if deployment_warning_summary != "none" else f"suggested={deployment_posture['suggested']}, selected={deployment_posture['selected']}, mode={deployment_posture['selection_mode']}" }`
...
- stage_02_output:
  - `{stage_02.name}`
- stage_02_5_output:
  - `{optional_stage_file_name}`
- stage_02_5_expected_from_phase1:
  - `{"true" if optional_stage_expected else "false"}`
- stage_02_5_decision_status:
  - `{optional_stage_decision_status}`
- stage_03_output:
  - `{stage_03.name}`
- stage_04_output:
  - `{stage_04.name}`
...
- phase_3_implementation_entry:
  - `{implementation_entry.name}`
- first_pass_audit_report:
  - `{first_pass_audit_path.name}`
- architecture_traceability_report:
  - `{trace_report_text.name}`
- trace_registry_validation_result:
  - `{trace_validation.name}`
- phase1_phase2_coverage_report:
  - `{phase1_phase2_coverage_path.name}`
- baseline_lock:
  - `baseline-lock.json`
- quality_check_report:
  - `quality-check-report.json`
- mermaid_validation_report:
  - `{mermaid_report_path.name}`
- cross_stage_consistency_report:
  - `{cross_stage_report_path.name}`

### 2.1 Fresh First-Pass Audit Snapshot
- audit_status:
  - `{first_pass_status}`
- audit_passed:
  - `{"true" if first_pass_passed else "false"}`
...
{conditional_matrix_md}

### 3.3 Not-Triggered Conditional Deliverables

{conditional_not_triggered_md}
...
- mandatory_row_count:
  - `{deliverable_summary.get('mandatory_count', len(mandatory_rows))}`
- triggered_conditional_row_count:
  - `{deliverable_summary.get('triggered_conditional_count', len(triggered_conditional_rows))}`
- not_triggered_conditional_row_count:
  - `{deliverable_summary.get('not_triggered_conditional_count', len(not_triggered_conditional_rows))}`
- checklist_rows_expected_for_this_run:
  - `{len(triggered_rows)}`
- judgment_matrix_row_count_actual:
  - `{len(triggered_rows)}`
- mapping_status:
  - `{"complete" if mapping_complete else "incomplete"}`

{checklist_mapping_md}
...
- overall_regression_gate:
  - `{quality_report['baseline_lock']['overall_regression_gate']}`
- regression_gate_reasoning:
  - compare against the preserved baseline lock; any negative delta requires explicit justification before the run can be treated as a stronger rerun
...
- total_structured_items:
  - `{review_bound['total_structured_items']}`
- review_bound_or_unknown_or_deferred_items:
  - `{review_bound['review_bound_or_unknown_or_deferred_items']}`
- ratio:
  - `{review_bound['ratio']}%`
- ceiling:
  - `30%`
- verdict:
  - `{review_bound['verdict']}`
- computation_method:
  - `{review_bound['computation_method']}`
- top_3_resolution_attempts_or_block_reasons:
  - preserve the highest-impact unresolved items in Stage-04 risk summary and do not silently upgrade them downstream
...
- runtime_contract_verdict:
  - `{trace_runtime_contract_verdict}`
...
### 8.1 Fine-Grained Trace Units
{fine_grained_trace_md}

### 8.2 Phase-1 to Phase-2 Coverage Contract
- total_phase1_trace_units:
  - `{coverage_summary.get('total_phase1_trace_units', 0)}`
- explicit_binding_rows:
  - `{coverage_summary.get('pass_rows', 0)}`
- inferred_only_rows:
  - `{coverage_summary.get('inferred_only_rows', 0)}`
- missing_rows:
  - `{coverage_summary.get('missing_rows', 0)}`
- overall_verdict:
  - `{coverage_summary.get('overall_verdict', 'fail')}`
- report_reference:
  - `{phase1_phase2_coverage_path.name}`
...
- strongest_output:
  - {stage_summaries['stage_01']['strongest_output']}
- weakest_output:
  - {stage_summaries['stage_01']['weakest_output']}
- dependency_realizability_scan_status:
  - `see Stage-01 critical_dependency_realizability_scan`
- quality_gate_result:
  - `{stage_summaries['stage_01']['quality_gate_result']}`
- progression_judgment:
  - `carry Stage-01 boundary/decision truth forward without semantic weakening`

### Stage-02 Summary
- outcome:
  - `present and evaluated`
- strongest_output:
  - {stage_summaries['stage_02']['strongest_output']}
- weakest_output:
  - {stage_summaries['stage_02']['weakest_output']}
- event_direct_driver_status:
  - `active` when `p2_architecture_event_model_driver` is present in Stage-02
- event_vocabulary_refs:
  - `stage-02-domain-module-service-decomposition.md#p2_architecture_event_model_driver`
- event_model_catalog_refs:
  - `stage-02-domain-module-service-decomposition.md#p2_architecture_event_model_driver`
- p3_event_handoff_status:
  - `ready-for-implementation-consumption` when Stage-03 carries `event_model_direct_consumption`
- review_bound_event_gaps:
  - see Stage-02 `review_bound_event_gaps`; unresolved event claims remain capped by P3/P4 evidence
- brownfield_or_ownership_conflict_status:
  - `see Stage-02 responsibility_matrix and dependency_collaboration_map`
- quality_gate_result:
  - `{stage_summaries['stage_02']['quality_gate_result']}`
- progression_judgment:
  - `use Stage-02 ownership and event semantics as the Stage-03 baseline`

### Stage-02.5 Optional Lane Summary
- outcome:
  - `{"present and evaluated" if optional_stage_present else "not present in this run"}`
- activation_state:
  - `{optional_stage_activation_state}`
- expected_from_phase1:
  - `{"true" if optional_stage_expected else "false"}`
- expectation_reason:
  - {optional_stage_expected_reason}
- decision_status:
  - `{optional_stage_decision_status}`
- quality_gate_result:
  - `{optional_stage_quality_display}`
- progression_judgment:
  - `{"provider/auth/fallback/mock-sandbox posture is now an explicit Stage-03 and Phase-3 input" if optional_stage_present else "no optional third-party integration lane was consumed in this run"}`

### Stage-03 Summary
- outcome:
  - `present and evaluated`
- strongest_output:
  - {stage_summaries['stage_03']['strongest_output']}
- weakest_output:
  - {stage_summaries['stage_03']['weakest_output']}
- substitute_boundary_and_dependency_realizability_status:
  - `see Stage-03 critical_external_dependency_realizability and substitute_boundary_plan`
- quality_gate_result:
  - `{stage_summaries['stage_03']['quality_gate_result']}`
- progression_judgment:
  - `use Stage-03 schema/API/contracts as the implementation-facing L3 baseline`

### Stage-04 Summary
- outcome:
  - `present and evaluated`
- strongest_output:
  - {stage_summaries['stage_04']['strongest_output']}
- weakest_output:
  - {stage_summaries['stage_04']['weakest_output']}
- readiness_calibration_and_review_reentry_status:
  - `see Stage-04 §4.1 Realizability Review`
- quality_gate_result:
  - `{stage_summaries['stage_04']['quality_gate_result']}`
- progression_judgment:
  - `use Stage-04 for realizability calibration, handoff usage rules, and work-package sequencing`
...
{markdown_table(
    ...
    [
        [
            ...
            row["check_item"],
            row["result"],
            row["inconsistency_found"],
            row["resolution_or_justification"],
        ]
...
- overall_consistency_verdict:
  - `{cross_stage_report['overall_consistency_verdict']}`

## 9.2 Quality Self-Assessment

| dimension | score (0-10) | evidence | gap_to_9.5 |
|---|---|---|---|
| architecture_decision_depth | {architecture_decision_depth_score} | `architecture_decisions_count={current_metrics['architecture_decisions_count']}, structured_adr_multi_alt_count={current_metrics.get('structured_adr_multi_alt_count', 0)}` | strengthen ADR alternatives depth and C4 context if below the active profile gate |
| domain_decomposition_clarity | {domain_decomposition_clarity_score} | `domain_count={current_metrics['domain_count']}, domain_event_count={current_metrics['domain_event_count']}` | add state diagrams and richer event detail where missing |
| specification_completeness | {specification_completeness_score} | `schema_table_count={current_metrics['schema_table_count']}, api_endpoint_count={current_metrics['api_endpoint_count']}` | close scenario/failure-semantic/storage-summary gaps |
| visual_representation_quality | {visual_representation_quality_score} | `diagram counts captured in baseline lock` | reach C4/state/gantt standards consistently |
| process_governance_rigor | {process_governance_rigor_score} | `baseline lock + regression rows + computed review-bound ratio` | complete standalone cross-stage and mermaid validation tooling |
| traceability_integrity | `{traceability_integrity_score}` | `{traceability_integrity_evidence}` | {traceability_integrity_gap} |

- aggregate_score:
  - `{aggregate_self_assessment_score:.1f}`
- gap_analysis_to_9.5:
  - {gap_analysis_to_95}
- top_3_improvement_actions:
{top_3_improvement_actions}
...
## 11. Blockers
- {"none" if formal_state != "blocked" else "quality-gate-fail or regression-gate-fail detected"}

## 12. Admission Recommendation
- recommended_formal_state:
  - `{formal_state}`
- stage_04_readiness_alignment:
  - stage_04_label = `{readiness_alignment.get('stage_04_label', 'not-declared')}`
  - quality_based_label = `{readiness_alignment.get('quality_based_label', 'unknown')}`
  - final_label = `{readiness_alignment.get('final_label', formal_state)}`
  - verdict = `{readiness_alignment.get('verdict', 'unknown')}`
- reasoning:
  - quality gates, baseline diff, traceability validation, and Engineering Spec Pack self-containment were evaluated together before recommending intake state
- closure_adjustment_reasons:
{("".join(f"  - {reason}\n" for reason in closure_adjustment_reasons)) if closure_adjustment_reasons else "  - none\n"}
- implementation_forbidden_assumptions:
  - downstream must not silently upgrade unresolved review-bound items to confirmed implementation truth
- unresolved_truth_to_preserve:
  - preserve all unresolved / review-bound items explicitly stated in the Phase-2 Stage outputs

## 13. Convergence Conclusion
- design_package_assembled:
  - `yes`
- engineering_spec_pack_converged:
  - `{"yes" if esp_check.get("passed") else "no"}`
