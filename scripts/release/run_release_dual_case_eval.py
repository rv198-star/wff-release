#!/usr/bin/env python3
"""
Prepare and optionally execute an isolated retained-case release-evaluation run.
"""
...
INSTALL_PACK_AGENTS_FILE = "AGENTS.md"
INSTALL_PACK_AGENTS_MAX_LINES = 120
INSTALL_PACK_AGENTS_REQUIRED_PHRASES = (
    "generated for this install pack",
    "Install whole directories from `skills/`",
    "Keep this install-pack root visible",
    "development / pre-production claims",
    "Do not copy the repository-level `AGENTS.md`",
)
REPO_LEVEL_AGENTS_LEAK_MARKERS = (
    "## Current Work Control",
    "Highest Project Principle: Workflow / Agentic / Evidence Boundary",
...
def build_phase_step_env(
    ...
    *,
    ...
    if phase == "phase3":
        ...
        vitest_max_forks = str(os.environ.get("PHASE3_VITEST_MAX_FORKS") or "").strip()
        if vitest_max_forks:
            env["PHASE3_VITEST_MAX_FORKS"] = vitest_max_forks
    return env
...
def audit_isolated_install_pack_agents_guidance(pack_root: Path) -> tuple[dict[str, Any], list[str]]:
    agents_path = pack_root / INSTALL_PACK_AGENTS_FILE
    if not agents_path.exists():
        return (
            {
                "path": INSTALL_PACK_AGENTS_FILE,
                "exists": False,
                "line_count": 0,
    ...
    missing_required_phrases = [
        phrase for phrase in INSTALL_PACK_AGENTS_REQUIRED_PHRASES if phrase not in text
    ]
    repo_level_markers = [marker for marker in REPO_LEVEL_AGENTS_LEAK_MARKERS if marker in text]

    failures: list[str] = []
    if line_count > INSTALL_PACK_AGENTS_MAX_LINES:
        failures.append(
            f"install-pack {INSTALL_PACK_AGENTS_FILE} is not concise: "
            f"{line_count} lines > {INSTALL_PACK_AGENTS_MAX_LINES}"
        )
    if missing_required_phrases:
        failures.append(
            f"install-pack {INSTALL_PACK_AGENTS_FILE} missing guidance phrases: "
            + ", ".join(missing_required_phrases)
        )
    if repo_level_markers:
        failures.append(
            f"repo-level {INSTALL_PACK_AGENTS_FILE} appears to have leaked into install pack: "
            + ", ".join(repo_level_markers)
        )
    ...
    return (
        {
            ...
            "max_allowed_lines": INSTALL_PACK_AGENTS_MAX_LINES,
            "missing_required_phrases": missing_required_phrases,
            "repo_level_markers": repo_level_markers,
        },
        failures,
    )
...
def prepare_isolated_release_workspace(
    *,
    ...
    if (isolated_release_root / INSTALL_PACK_AGENTS_FILE).exists():
        agents_guidance, agents_failures = audit_isolated_install_pack_agents_guidance(isolated_release_root)
        if agents_failures:
            isolation_failures.append(
                "invalid install-pack AGENTS.md in isolated release copy: "
                + "; ".join(agents_failures)
            )
    ...
    result = {
        ...
        "isolation_checks": {
            "agents_absent": not (isolated_release_root / INSTALL_PACK_AGENTS_FILE).exists(),
            "install_pack_agents_guidance": agents_guidance,
...
def read_phase3_runtime_reset_summary(backend_run_root: Path) -> dict[str, Any]:
    events: list[dict[str, Any]] = []
    events_root = backend_run_root / "runtime-reset-events"
    if events_root.exists() and events_root.is_dir():
        for path in sorted(events_root.glob("runtime-reset-events-*.json")):
    ...
    passed_duration_by_stage_ms: dict[str, int] = {}
    passed_count_by_stage: dict[str, int] = {}
    fallback_count = 0
    for event in events:
        ...
        if not stage:
            continue
        if "fallback" in stage and status == "passed":
            fallback_count += 1
        if status != "passed":
            continue
        passed_count_by_stage[stage] = passed_count_by_stage.get(stage, 0) + 1
        duration_ms = safe_int(event.get("duration_ms") or 0)
        passed_duration_by_stage_ms[stage] = passed_duration_by_stage_ms.get(stage, 0) + duration_ms

    return {
        "event_count": len(events),
        "fallback_count": fallback_count,
        "passed_count_by_stage": passed_count_by_stage,
        "passed_duration_by_stage_ms": passed_duration_by_stage_ms,
    }
...
def read_phase3_runtime_cost_summary(phase3_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "runtime_reset": read_phase3_runtime_reset_summary(backend_run_root),
    }
...
def build_phase_command_plan(
    *,
    ...
    output_locale: str,
    depth_mode: str,
    phase1_thinking_value_gain_mode: str = "off",
    phase1_thinking_value_gain_output_profile: str = "coverage_rich",
    phase2_thinking_value_gain_mode: str = "off",
    phase2_thinking_value_gain_output_profile: str = "coverage_rich",
    phase3_thinking_value_gain_mode: str = "off",
    phase3_thinking_value_gain_output_profile: str = "coverage_rich",
    stop_after: str,
    ...
    if phase1_thinking_value_gain_mode != "off":
        phase1_command.extend(["--thinking-value-gain-mode", phase1_thinking_value_gain_mode])
        phase1_command.extend(["--thinking-value-gain-output-profile", phase1_thinking_value_gain_output_profile])
    ...
    if phase2_thinking_value_gain_mode != "off":
        phase2_command.extend(["--thinking-value-gain-mode", phase2_thinking_value_gain_mode])
        phase2_command.extend(["--thinking-value-gain-output-profile", phase2_thinking_value_gain_output_profile])
    steps.append(
        {
            "phase": "phase2",
    ...
    if phase3_thinking_value_gain_mode != "off":
        phase3_command.extend(["--thinking-value-gain-mode", phase3_thinking_value_gain_mode])
        phase3_command.extend(["--thinking-value-gain-output-profile", phase3_thinking_value_gain_output_profile])
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Prepare and optionally execute an isolated Release SKILLS retained-case evaluation"
    )
    parser.add_argument(
        "--release-root",
        default=".",
    ...
    parser.add_argument(
        "--case-input-root",
        required=True,
        help="Case-input root under the selected release root, or an absolute path to external retained inputs.",
    )
    ...
    parser.add_argument(
        "--phase1-thinking-value-gain-mode",
        choices=("off", "full-use"),
        default="off",
        help="Phase-1 TVG strategy to pass through to run_phase1_full_trial.py; defaults to off",
    )
    parser.add_argument(
        "--phase1-thinking-value-gain-output-profile",
        choices=THINKING_VALUE_GAIN_OUTPUT_PROFILES,
        default="coverage_rich",
        help="Phase-1 Mindthus TVG output profile to pass through when TVG full-use is enabled",
    )
    parser.add_argument(
        "--phase2-thinking-value-gain-mode",
        choices=("off", "full-use"),
        default="off",
        help="Phase-2 TVG strategy to pass through to run_phase2_first_version.py; defaults to off",
    )
    parser.add_argument(
        "--phase2-thinking-value-gain-output-profile",
        choices=THINKING_VALUE_GAIN_OUTPUT_PROFILES,
        default="coverage_rich",
        help="Phase-2 Mindthus TVG output profile to pass through when TVG full-use is enabled",
    )
    parser.add_argument(
        "--phase3-thinking-value-gain-mode",
        choices=("off", "full-use"),
        default="off",
        help="Phase-3 TVG strategy to pass through to run_phase3_first_version.py; defaults to off",
    )
    parser.add_argument(
        "--phase3-thinking-value-gain-output-profile",
        choices=THINKING_VALUE_GAIN_OUTPUT_PROFILES,
        default="coverage_rich",
        help="Phase-3 Mindthus TVG output profile to pass through when TVG full-use is enabled",
    )
...
def main() -> int:
    ...
    for case_index, case_name in enumerate(args.cases):
        ...
        steps = build_phase_command_plan(
            ...
            phase1_thinking_value_gain_mode=args.phase1_thinking_value_gain_mode,
            phase1_thinking_value_gain_output_profile=args.phase1_thinking_value_gain_output_profile,
            phase2_thinking_value_gain_mode=args.phase2_thinking_value_gain_mode,
            phase2_thinking_value_gain_output_profile=args.phase2_thinking_value_gain_output_profile,
            phase3_thinking_value_gain_mode=args.phase3_thinking_value_gain_mode,
            phase3_thinking_value_gain_output_profile=args.phase3_thinking_value_gain_output_profile,
