...
`wff-req` also accepts `packet_subtype: existing-system-change` under the same `P1 source input packet` shell. This is a generic compatibility entry for a change to an existing system; it is not PhaseX-specific. The producer may be PhaseX, a human reviewer, or another upstream preparation step.

When preparing this subtype, use `docs/phases/phase-1/phase-1-existing-system-change-source-packet-template-v0.1.md` as the input template.

Read these packet sections before official Phase-1 execution:

- `P1 Source Brief`
- `Demand Change Evaluation` when `packet_subtype: existing-system-change`
- `Demand Clarification Questions` and `Demand Clarification Addendum` when present
...
For `packet_subtype: existing-system-change`, additionally preserve the split between:

- current system state and target change
- observed business facts and inferred business semantics
- legacy behaviors to preserve and new behaviors to introduce
- explicit unknowns, source conflicts, and non-goals

Current state is context, not automatically a target requirement. Inferred semantics must stay labeled as inference until confirmed. Legacy behavior preservation should become requirement constraints, acceptance boundaries, or review-bound items. Technical, database, interface, performance, and architecture facts may inform business pressure and P2 handoff clues, but P1 must not judge or resolve them as architecture truth.

For `packet_subtype: existing-system-change`, run a light `Demand Change Evaluation` after source admission and before normal P1 demand convergence. P1 evaluates demand clarity and business pressure only; it must not judge architecture, database, code, or implementation plan.

For `packet_subtype: existing-system-change`, a `Demand Clarification Addendum` may appear between PX and P1. Use it as optional demand clarification evidence. If answers are present, let them sharpen P1's target, acceptance boundary, priority workflow, compatibility promise, and remaining review-bound items. If answers are missing, continue only with visible review-bound gaps and conservative defaults; do not block P1 solely because the addendum is unanswered.

The addendum is not owner sign-off, UAT, market validation, production readiness, or production risk acceptance.

The evaluation must make these points explicit:

- `Change Intent`: what the user wants to add, change, replace, improve, or narrow.
- `Business Impact`: why this change matters now and what value, risk, loss, or operating pressure it addresses.
- `Affected Users / Workflows`: which users, roles, workflows, or decision points are affected.
- `Acceptance Pressure`: what would make the change acceptable or unacceptable from a business, user, operations, compliance, or review standpoint.
- `Non-Goals / Scope Boundaries`: what this change should not cover.
- `Proceed Decision`: choose `proceed-to-P1`, `return-to-intake`, or `review-bound-provisional`, with a short reason and visible gaps.

Use `return-to-intake` when the target change or affected business meaning is too unclear. Use `review-bound-provisional` only when the user accepts visible uncertainty. Use `proceed-to-P1` only when the demand is clear enough for P1 to converge without inventing product truth.
...
- for `packet_subtype: existing-system-change`, must not treat the existing implementation, database shape, API shape, or performance observation as P1-owned architecture truth

### P1 PRD Claim Authority Surface

Official Phase-1 PRD runs must emit `.phase1-evidence/<prd-stem>.claim-surface.json` before PRD claim-control validation.

The PRD Markdown is a rendered human-review view. It may carry trace IDs, tables, Mermaid, and reader-facing prose, but it must not become source authority for the same product claims it renders.

For the PRD claim-control sidecar, the claim-control sidecar must prefer the compiled claim surface built from P1 source / stage contexts. A rendered PRD compatibility fallback is allowed only when a claim family is not yet available from source / stage surfaces; fallback rows must stay visibly compatibility-bound and must not become source authority.
...
With Mindthus `v0.6.2`, default P1 long-document quality work should use the
TVG `coverage_rich` output profile because PRD generation needs enough review
and handoff structure to preserve business thickness. The profile is an exit
delivery bias, not permission to expand weak material: first pass the TVG
Thinking Thickness gate, then improve Grounded Insight Yield and Value Density.

- define the value exit gate before each stage artifact is deepened
- keep decision questions, decision standards, success / failure conditions,
  alternatives, adoption risks, evidence boundaries, and review-bound unknowns
  visible when they materially help product/design/architecture handoff
...
| Inspect why depth/regression is weak | read generated gate, depth-runtime, and optional excellence-regression reports |
