...
With Mindthus `v0.6.2`, default P2 long-document quality work should use the
TVG `coverage_rich` output profile because ESP handoff needs enough design
review and implementation structure to preserve architecture thickness. The
profile is an exit delivery bias, not permission to add low-value architecture
ceremony: first pass the TVG Thinking Thickness gate, then improve Grounded
Insight Yield and Value Density.

The Phase-2 runners expose `--thinking-value-gain-mode off|full-use` and
...
prefer the default `coverage_rich` profile for long ESP / architecture handoff
quality work.

- define the design value exit gate before deepening each artifact unit
- keep decision questions, trade-off standards, failure paths, rollback /
  migration constraints, evidence boundaries, and review-bound unknowns visible
  when they materially improve implementation or validation handoff
...
### Optional Existing-System Architecture Change Side Branch

When the case is an existing-system architecture change, `wff-arch` may also consume a `P2 Existing-System Architecture Change Intake Packet` with `packet_subtype: existing-system-architecture-change`.

This is not a new Phase-2 flow. P1 PRD and `Phase-2 Design Input Contract` remain the authority input for demand, scope, trace, and acceptance pressure. The side branch only supplies existing-system architecture facts, constraints, conflicts, unknowns, migration pressure, and implementation guardrails.

Workflow only controls side-branch order and claim guards:

1. detect the optional side branch
2. require `Architecture Change Impact Triage`
3. require `Architecture Change Design`
4. keep AC-3 / AC-4 from silently becoming implementation-ready

Agentic owns the architecture judgment:

- decide the real impact level
- interpret DB / API / service / runtime / external caller blast radius
- choose compatibility, migration, rollback, adapter, and decision-gate posture
- map the result back into normal Stage-01/02/03/04 expression

Use the template:

- `docs/phases/phase-2/phase-2-existing-system-architecture-change-intake-template-v0.1.md`

P2 may consume the side branch only after it preserves these boundaries:

- it must not consume raw PhaseX scan tables as architecture truth
- it must not understand PhaseX internal skill names or scan structure
- it must not replace P1 demand truth with technical findings
- it must not decide code-level edits, file edits, class names, or concrete refactoring steps
- it must not generate P3 ActionCards directly
- it must preserve observed / inferred / unknown / conflict / review-bound labels

`Architecture Change Impact Triage` must classify the change:

| Level | Meaning | P2 handling |
|---|---|---|
| `AC-1` | Local internal adjustment that does not break DB/API/external contracts | P2 may design directly and record validation points |
| `AC-2` | Contract-visible change with a credible compatibility path | P2 may design only with compatibility, migration, and rollback strategy |
| `AC-3` | High-risk change such as database foundation change, public API breaking change, service split, or cross-system impact | P2 may produce options, risk, protection strategy, and an explicit architecture decision gate |
| `AC-4` | Platform-level rebuild, technology strategy replacement, or broad system rewrite | P2 should block the normal implementation path or require a separate architecture decision |

`AC-3 / AC-4 must not be promoted as ready-for-P3` without an explicit architecture decision and a visible claim ceiling.

If the required decision is missing, mark AC-3 as `architecture-decision-required` and AC-4 as `blocked` or routed to a separate architecture decision path.

For existing-system architecture changes, additive compatibility is the default safety posture for this side branch. Prefer adding, wrapping, deprecating, archiving, downgrading, migrating, and rolling back safely before deleting or replacing existing contracts. This is not a global Phase-2 principle; it is only the default safety posture for existing-system change work. A destructive replacement requires an explicit architecture decision. Owner confirmation is optional evidence; if no owner is available, keep the missing confirmation review-bound and choose the conservative compatibility-preserving route.

`Architecture Change Design` then maps the triage result into the existing Stage surfaces:

- Stage-01 owns boundary impact, impact level, compatibility posture, and destructive-change decision gates.
- Stage-02 owns additive boundaries, adapter boundaries, preserved module/service contracts, and deprecated or archived boundaries.
- Stage-03 owns schema migration posture, API compatibility strategy, versioning/adapters, and rollback-safe contracts.
- Stage-04 owns migration sequence, protection test obligations, rollback readiness, architecture decision gates, and P3 handoff constraints.
...
Keep generated case outputs there rather than inside `reference-packages/` or repository validation fixtures.
...
### Quantitative And Realization Consistency
- Capacity estimates in Stage-01 must be reflected in Stage-03's storage strategy
- Every Stage-02 aggregate/object must have an explicit Stage-03/04 realization path:
  schema table, projection/read model, contract/API, event/log stream, external mapping,
  explicit non-persistent/computed decision, or review-bound deferral.
- Schema table count vs Stage-02 aggregate/object count is diagnostic context only.
  Table-count parity is not required when realization coverage is explicit through
  another accepted path.
