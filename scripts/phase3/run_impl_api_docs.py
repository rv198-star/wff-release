#!/usr/bin/env python3
from __future__ import annotations

from pathlib import Path
import sys

SCRIPTS_ROOT = Path(__file__).resolve().parents[1]
if str(SCRIPTS_ROOT) not in sys.path:
    sys.path.insert(0, str(SCRIPTS_ROOT))

import argparse

from common.output_language import resolve_output_locale
from phase3.api_doc_generation import generate_phase3_api_docs
from phase3.contract_tools import load_openapi_document
from phase3.impl_context import emit_summary


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate Phase-3 API documentation capability artifacts")
    parser.add_argument("--baseline-openapi", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--title", default="Phase-3 API Documentation")
    parser.add_argument("--candidate-openapi", default="")
    parser.add_argument("--output-locale", default=resolve_output_locale())
    return parser


def main(argv: list[str] | None = None) -> int:
    args = build_parser().parse_args(argv)
    summary = generate_phase3_api_docs(
        baseline_openapi=load_openapi_document(Path(args.baseline_openapi).resolve()),
        output_dir=Path(args.output_dir).resolve(),
        title=args.title,
        candidate_openapi=load_openapi_document(Path(args.candidate_openapi).resolve()) if args.candidate_openapi else None,
        output_locale=resolve_output_locale(args.output_locale),
    )
    summary = {"quality_gate": "pass", **summary}
    return emit_summary(summary)


if __name__ == "__main__":
    raise SystemExit(main())
